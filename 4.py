...
requiredFields = ['byr', 'iyr', 'eyr', 'hgt', 'hcl', 'ecl', 'pid'] # optional 'cid'
...
for passport in passports:
    values = {}
    for property in re.split('[\n ]+', passport):
        (name, val) = property.split(':')
        values[name] = val
    
    # normalization
    
    if all(key in values.keys() for key in requiredFields):
        validPassports += 1

print(f'passports={len(passports)} validPassports={validPassports}')
...
for passport in passports:
    ...
    # normalization
    values['hgtmatch'] = re.search('^(\d+)(in|cm)$', values['hgt']) if 'hgt' in values else None
    
    if all(key in values.keys() for key in requiredFields) \
        and int(values['byr']) >= 1920 and int(values['byr']) <= 2002 \
        and int(values['iyr']) >= 2010 and int(values['iyr']) <= 2020 \
        and int(values['eyr']) >= 2020 and int(values['eyr']) <= 2030 \
        and values['hgtmatch'] and ((values['hgtmatch'][2] == 'cm' and int(values['hgtmatch'][1]) >= 150 and int(values['hgtmatch'][1]) <= 193) \
            or (values['hgtmatch'][2] == 'in' and int(values['hgtmatch'][1]) >= 59 and int(values['hgtmatch'][1]) <= 76)) \
        and re.search('^#[0-9a-f]{6}$', values['hcl']) \
        and values['ecl'] in ['amb', 'blu', 'brn', 'gry', 'grn', 'hzl', 'oth'] \
        and re.search('^\d{9}$', values['pid']):
        validPassports += 1
