...
for password in passwords:
    (minUses, maxUses, character, passText) = re.split(r'[: \-]+', password)
    minUses = int(minUses)
    maxUses = int(maxUses)
    ...
    if charCounter >= minUses and charCounter <= maxUses:
        correctPasswords += 1

print(f'correct passwords: {correctPasswords}')

# part 2
correctPasswords = 0

for password in passwords:
    (firstPos, secondPos, character, passText) = re.split(r'[: \-]+', password)
    firstPos = int(firstPos) - 1
    secondPos = int(secondPos) - 1

    if (passText[firstPos] == character) != (passText[secondPos] == character):
        correctPasswords += 1
