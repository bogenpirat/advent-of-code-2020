input = """shiny aqua bags contain 1 dark white bag.
...
rules = {}
for ruleText in rulesText:
    m = regex.match('(\S+ \S+) bags contain( (\d+) (\S+ \S+) bags?[,.])+', ruleText)
    empty_m = regex.match('(\S+ \S+) bags contain no other bags.', ruleText)
    if m:
        name = m.captures(1)[0]
        subBags = m.captures(4)
        rules[name] = subBags
    elif empty_m:
        name = empty_m.captures(1)[0]
        rules[name] = []

containers = set()
def can_contain(tree, color_stack):
    for color, containedColors in tree.items():
        if color == 'shiny gold':
            containers.update(color_stack)
        else:
            subTree = {}
            for containedColor in containedColors:
                subTree[containedColor] = rules[containedColor]
            can_contain(subTree, [*color_stack, color])

can_contain(rules, [])

print(f'len(containers)={len(containers)}')
...
for ruleText in rulesText:
    ...
    if m:
        name = m.captures(1)[0]
        subBagNames = m.captures(4)
        nums = [int(x) for x in m.captures(3)]
        subBags = {}
        for i in range(0, len(subBagNames)):
            subBags[subBagNames[i]] = nums[i]
        rulesNums[name] = subBags
    elif empty_m:
        name = empty_m.captures(1)[0]
        rulesNums[name] = 1

def must_contain(tree):
    numBags = 0
    
    if type(tree).__name__ == 'dict':
        for color, num in tree.items():
            numBags += num * (must_contain(rulesNums[color]) + 1)
    else:
        numBags = 0

    return numBags

print(must_contain(rulesNums['shiny gold']))